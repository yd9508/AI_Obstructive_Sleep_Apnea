...
def coefVecEstimate1(i, x_ik, residual_i, betaL, currCoefVec_i, in_time):
    trans_time = time.time() - in_time
    trans_data = (sys.getsizeof(i) + sys.getsizeof(x_ik) +
                  sys.getsizeof(residual_i) + sys.getsizeof(betaL) +
                  sys.getsizeof(currCoefVec_i) + sys.getsizeof(in_time))
...
def tempLstGenerate1(i, x_i, residual_i, betaL, numPredictors, in_time):
    trans_time = time.time()-in_time
    # 1705521783.6395488
    trans_data = (sys.getsizeof(i) + sys.getsizeof(x_i) +
                  sys.getsizeof(residual_i) + sys.getsizeof(betaL) +
...
def yhatGenerate1(i, tempLst_i, numPredictors, coefestimate, x_i, in_time):
    trans_time = time.time()-in_time
    trans_data = (sys.getsizeof(i) + sys.getsizeof(tempLst_i) +
                  sys.getsizeof(numPredictors) + sys.getsizeof(coefestimate) +
                  sys.getsizeof(x_i) + sys.getsizeof(in_time))
...
def sseGenerate1(i, tempLst_i, numPredictors, residual_i, in_time):
    trans_time = time.time()-in_time
    trans_data = (sys.getsizeof(i) + sys.getsizeof(tempLst_i) +
                  sys.getsizeof(numPredictors) + sys.getsizeof(residual_i) +
                  sys.getsizeof(in_time))
...
def federatedFunctionalGradBoostLSA(x, y, betaList, boost_control, step_length, ray_control = True):
    trans_time_list = []
    trans_data_total = 0
    numworkers = len(y)
    init = y[0].mean()
    ...
    while j <= boost_control:
        ...
        if ray_control:
            ...
            tempLstIdx = [tempLstGenerate1(i, x[i], residual[i], betaList, numPredictors, cur_time) for i in
                            range(numworkers)]
        cur_time = time.time()
        ...
        if ray_control:
            ...
            yhatIdx = [
            yhatGenerate1(i, tempLst[i], numPredictors, coefestimate, x[i], cur_time) for i in
                        range(numworkers)]
        ...
        if ray_control:
            ...
            sseIdx = [sseGenerate1(i, tempLst[i], numPredictors, residual[i], cur_time) for i in
                        range(numworkers)]
        cur_time = time.time()
