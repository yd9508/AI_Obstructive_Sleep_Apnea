...
def verticalFederatedFunctionalGradBoost(x, y, betaList, boost_control, step_length, epsilon, delta, DP=True,
                                         Clipping=True):
    """
    ...
    while num_iteration <= boost_control:
        ...
        tempLst = []
        # The following loop is where we shall apply parallel computing.
        for num_p in range(numPredictors):
            lin = linmod(x[num_p], residual, betaList)
            sse = inprod(((predit_linmod(lin, x[num_p]) - residual) * (predit_linmod(lin, x[num_p]) - residual))).sum()
            tempLst.append([lin, sse])
