...
def linmod(xfdobj, yfdobj, betalist, wtvec=None):
    ...
    # solve the equations
    Cmatpinv = np.linalg.pinv(Cmat)
    coefvec = Cmatpinv.dot(Dmat)
...
def pred_gradboostVFL1(res):
    length = len(res)
    y_pred = res[0] * 0.5
    for i in range(1, length):
        step_length = res[i].step_length
        y_pred = step_length * res[i].yhatfdobj + y_pred
    return y_pred


def pred_gradboostVFL2(res, x):
    length = len(res)
    coefdim = x[0].coef.shape
    res[0].coef = np.repeat(res[0].coef[:, 0], repeats=coefdim[1], axis=1)

    y_pred = res[0] * 0.5
    for i in range(1, length):
        id = res[i].id
        step_length = res[i].step_length
        y_pred = step_length * predit_linmod(res[i], x[id]) + y_pred
